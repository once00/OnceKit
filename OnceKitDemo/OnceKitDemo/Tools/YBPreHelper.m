...
+ (UIImageView *)findHairlineImageViewUnder:(UIView *)view {//消除下划线
    ...
    if ([view isKindOfClass:UIImageView.class] && view.bounds.size.height <= 1.0) {
        view.hidden = NO;
        UIImageView * lineImgView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, A_pixel)];
        lineImgView.image = [YBPreHelper imageWithColor:Theme_Color];
...
+ (void)setNavState:(NSString *)navState vc:(UIViewController *)vc{
    ...
    if ([navState isEqualToString:NavState_White]) {
        ...
        [YBPreHelper findHairlineImageViewUnder:vc.navigationController.navigationBar];
        [vc.navigationController.navigationBar setTintColor:Theme_Color];
        [vc.navigationController.navigationBar setTitleTextAttributes:
         [NSDictionary dictionaryWithObjectsAndKeys:[YBPreHelper colorWithHexString:@"555555"], NSForegroundColorAttributeName, [UIFont fontWithName:FONT_NAME size:18.0], NSFontAttributeName, nil]];
    }else if ([navState isEqualToString:NavState_Green]) {
        [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
        [vc.navigationController.navigationBar setBarTintColor:[YBPreHelper colorWithHexString:@"#80d136"]];
        //#1992f5
...
//限制只能填写金额的textfield
+ (void)decimalNumberTextField:(UITextField *)textField maxMoney:(NSString *)maxMoney exceedMax:(void (^)()) exceedMax {
...
+ (NSString *)timeIntervalFromLastTime:(NSString *)lastTime
    ...
    return [YBPreHelper timeIntervalFromLastTime:lastDate ToCurrentTime:currentDate];
}
